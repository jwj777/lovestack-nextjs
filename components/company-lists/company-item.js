...
function CompanyItem({ company, index }) {
  return (
  <Box 
    key={company.companyName + '__' + index} className={styles.companyItem}
    display='flex'
  ...
  >
    <Box>
      <Box display="flex" alignItems={{ base: "center", lg: "center" }} justifyContent="flex-start" mb='7'>
        ...
        <Box>
          <Link href={"/companies/" + company.slug}>
            <Text fontSize={{ base: '1.6rem', md: '2rem' }} lineHeight='2.7rem' fontWeight="700" mb='0' _hover = {{ textDecoration: 'none' }}>
              {company.companyName}
            </Text>
          </Link>
